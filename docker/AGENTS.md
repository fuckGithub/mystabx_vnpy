...
| Backend | fastapiadmin-backend  | 18080   | FastAPI 应用（容器内 18080，对齐 mystabx）|
...
- **服务器（本仓库运维 / Redis）**：47.102.208.231 — 宿主机 `redis-server`（非 compose）；方式 B：`REDIS_HOST=127.0.0.1`，库 `mystabx_vnpy`，端口 18080
...
- **端口口径**：容器内统一 **18080**（`SERVER_PORT`，对齐 mystabx `STABX_PORT`），改端口必须同步
  `Dockerfile` 的 `EXPOSE`、编排的 `SERVER_PORT`、`nginx.conf` 的 `proxy_pass`
  （`backend/tests/test_docker_deploy_config.py` 已锁住）。
- **MySQL 默认库名**：`mystabx_vnpy`（用户默认 `root`）；见 `docker/.env.example`。
- **ECS 注意**：线上交易服务已占用 **18080** 时，勿直接把新栈绑到同端口；先用模板
  `backend/env/.env.prod.example` / `.env.ecs.example` 对齐 MySQL/Redis，切流前再改端口。
