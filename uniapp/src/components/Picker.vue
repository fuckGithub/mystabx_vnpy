...
import type { PickerViewInstance } from '@wot-ui/ui/components/wd-picker-view/types'
...
function handleColumnChange(
  ...
  updatePickerColumns(item.value, columnIndex)
  resolve()
}
 ...
 * 动态更新所有后续列的数据
 */
function updatePickerColumns(parentId: string | number, columnIndex: number) {
  const nextColumnIndex = columnIndex + 1
  const children = findChildren(parentId, props.data)

  if (children.length > 0 && nextColumnIndex < 3) {
    const newColumns = [...pickerColumns.value]
    newColumns[nextColumnIndex] = children as any
    for (let i = nextColumnIndex + 1; i < newColumns.length; i++) {
      newColumns[i] = []
    }
    pickerColumns.value = newColumns
    updatePickerColumns(children[0].value, nextColumnIndex)
  }
}
