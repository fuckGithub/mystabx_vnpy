...
function handleAvatarConfirm(event: any) {
  ...
  // 使用 UserAPI 的头像上传方法
  UserAPI.uploadCurrentUserAvatar({ file: tempFilePath }).then((fileInfo: UploadFileResult) => {
    const avatarForm = {
...
// 本页面中所有的校验规则
type FormRules = Record<string, { required: boolean; message: string }[]>
  ...
  <view class="app-container">
    <wd-card v-if="userProfile" custom-style="margin-top: 20rpx" class="theme-card">
      <wd-cell-group border>
        <wd-cell class="avatar-cell" title="头像" is-link center @click="avatarUpload">
          <template #title>
