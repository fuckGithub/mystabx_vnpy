...
export interface FestivalConfig {
  ...
  count?: number
  /**
   * 礼花连发间隔（毫秒）。不填默认 1000。可改小以营造连续绽放感（如雪花「暴风」）。
   */
  ...
  /**
   * 常驻兜底公告（如全年系统提示）。匹配优先级最低：**普通节日 > 周末提示 > 常驻**
   */
...
// 应用配置
export interface AppEnv extends SystemConfig {
  // 环境配置
  env: EnvConfig
  // 开发模式
  isDev: boolean
