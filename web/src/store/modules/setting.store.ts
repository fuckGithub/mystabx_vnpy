...
export const useSettingsStore = defineStore(
  ...
  () => {
    ...
    const getMenuTheme = computed((): MenuThemeType => {
      const list = AppEnv.themeList.filter((item) => item.theme === menuThemeType.value)
      if (isDark.value) {
        return AppEnv.darkMenuStyles[0]
      } else {
        return list[0]
      }
    })
    ...
    function updateSetting<K extends keyof typeof settingsMap>(key: K, value: boolean | string): void {
