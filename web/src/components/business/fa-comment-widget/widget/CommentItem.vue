  ...
  <li>
    <div>
      ...
      <div class="flex-c mt-2.5">
        ...
        <div class="ml-5 text-xs text-g-700 c-p select-none hover:text-theme" @click="toggleReply(comment.id)">
          回复
        </div>
      </div>
    </div>
    ...
    <ElForm v-if="showReplyForm === comment.id" @submit.prevent="handleSubmit" class="mt-4">
      ...
      <ElFormItem prop="content">
        <ElInput v-model="replyContent" placeholder="你的回复..." type="textarea" :rows="3" clearable />
      </ElFormItem>
      <ElFormItem>
...
const randomColor = () => {
  ...
  do {
    const index = Math.floor(Math.random() * AppEnv.systemMainColor.length)
    newColor = AppEnv.systemMainColor[index]
