          ...
          <template v-for="col in columns" :key="col.prop || col.type">
            ...
            <ElTableColumn v-else-if="col.type === 'expand'" v-bind="cleanColumnProps(col)">
              <template #default="{ row: expandRow }">
                <slot v-if="$slots.expand" name="expand" v-bind="{ row: expandRow }" />
                <component v-else :is="col.formatter ? col.formatter(expandRow) : null" />
              </template>
            </ElTableColumn>
...
const { isBorder, isZebra, tableSize, isFullScreen, isHeaderBackground, isRowDrag } = storeToRefs(tableStore)
