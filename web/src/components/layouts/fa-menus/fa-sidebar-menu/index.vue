...
<template>
  <div class="layout-sidebar" v-if="showLeftMenu || isDualMenu" :class="{ 'no-border': menuList.length === 0 }">
    ...
    <div
      ...
      <FaIconButton class="switch-btn size-10" icon="ri:arrow-left-right-fill" @click="toggleDualMenuMode" />
    </div>
    ...
    <div
      ...
      <div class="dual-menu-collapse-btn" v-if="isDualMenu" @click="toggleMenuVisibility">
        <FaSvgIcon class="text-g-500/70" :icon="menuOpen ? 'ri:arrow-left-wide-fill' : 'ri:arrow-right-wide-fill'" />
      </div>
...
const { getMenuOpenWidth, menuType, uniqueOpened, dualMenuShowText, menuOpen, getMenuTheme, showAppLogo } =
...
const menuList = computed(() => {
  ...
  if (!currentMenu && allMenus.length > 0) {
    currentMenu = allMenus.find((menu) => !menu.meta?.isHide && menu.children?.length) ?? allMenus[0]
  }
