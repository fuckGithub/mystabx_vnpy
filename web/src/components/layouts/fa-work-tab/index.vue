...
<template>
  <div
    ...
    <div
      ...
      <div class="worktab-scroll-wrap flex flex-1 min-w-0 items-center overflow-hidden" ref="scrollRef">
        <ul
          ...
          <li
            ...
            <FaSvgIcon v-if="item.fixedTab" icon="ri:pushpin-2-fill" class="worktab-pin shrink-0 text-sm ml-0.5" />
            <span
    ...
    <FaMenuRight ref="menuRef" :menu-items="menuItems" :menu-width="140" :border-radius="10" @select="handleSelect" />
  </div>
</template>
...
.worktab-tab.worktab-tab--card {
  gap: 6px;
  background: color-mix(in srgb, var(--el-bg-color) 94%, var(--el-text-color-regular) 6%) !important;
...
.dark .worktab-tab.worktab-tab--card {
  background: color-mix(in srgb, var(--el-bg-color-overlay) 92%, var(--el-text-color-regular) 8%) !important;
