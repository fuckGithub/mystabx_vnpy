    ...
    <ElUpload
      ...
      <template #default>
        <template v-if="internalFileList && internalFileList.length > 0 && internalFileList[0].url">
          ...
          <ElIcon v-if="!props.disabled" class="single-upload__delete-btn" @click.stop="handleDelete">
            <CircleCloseFilled />
          </ElIcon>
        </template>
        <template v-else>
...
async function onCropConfirm(dataURL: string) {
  try {
    const ext = props.cropFileType === 'png' ? 'png' : props.cropFileType === 'webp' ? 'webp' : 'jpg'
