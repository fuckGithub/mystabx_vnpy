...
export function useCeremony() {
  ...
  const buildWeekendPrompt = (dateStr: string): FestivalConfig => {
    ...
    return {
      ...
      date: dateStr,
      image: '',
      scrollText: isSat ? '🌤️ 周末到啦，适当放松，记得分支与构建状态也要照顾好～' : '🌅 周日休整好，新一周迭代加油。',
      skipFireworks: true,
      count: 3,
    }
  }
  ...
  const currentFestivalData = computed((): FestivalConfig | undefined => {
    ...
    const matches = mergedFestivalEntries.value.filter((item) => isDateInRange(currentDate, item.date, item.endDate))
  ...
  const openFestival = () => {
    ...
    if (cur.skipFireworks) {
      if (typeof localStorage !== 'undefined' && localStorage.getItem(festivalScrollDismissKey(cur)) === '1') {
        return
      }
      settingStore.setShowFestivalText(true)
      return
