...
const filteredTreeData = computed<TreeNode[]>(() => {
  ...
  const cloneFilter = (node: TreeNode, parents: string[] = []): TreeNode | null => {
    ...
    const children = (node.children || []).map((c) => cloneFilter(c, nextParents)).filter(Boolean) as TreeNode[]
...
async function handleSynchDb(row: GenTableSchema): Promise<void> {
  ...
  try {
    ...
    const subHtml =
      preview?.sub_table_name && preview?.sub ? renderHtml(`子表：${preview.sub_table_name}`, preview.sub) : ''
...
async function handleImportTable(): Promise<void> {
  ...
  try {
    ...
    if (tables.value.length === 1) {
      ...
      const importedTable = list.find((t: GenTableSchema) => t.table_name === tables.value[0].table_name)
      if (importedTable) {
