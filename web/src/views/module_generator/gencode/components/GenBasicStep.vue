    ...
    <ElRow :gutter="12" class="mb-3">
      <ElCol :span="24">
        <ElCard shadow="never" class="gen-echo-card">
          ...
          <div v-if="info.sub_table_name && info.sub_table_fk_name && !info.master_sub_hint" class="gen-echo-warn">
            将额外生成子表代码（不创建子表菜单）
          </div>
        </ElCard>
      </ElCol>
          ...
          <ElRow :gutter="16">
            ...
            <ElCol :span="12">
              <ElFormItem prop="package_name">
                <template #label>
                  包名
                  <ElTooltip content="插件包名（plugin 顶层目录）。三段式示例：module_example" placement="top">
            ...
            <ElCol :span="12">
              <ElFormItem prop="business_name">
                <template #label>
                  业务名
                  <ElTooltip content="模块下第三层目录（可为空）。示例：demo01；留空表示仅到模块目录" placement="top">
                    <ElIcon><QuestionFilled /></ElIcon>
                  </ElTooltip>
                </template>
                <ElInput v-model="info.business_name" placeholder="例如 demo01（可留空）" clearable />
              </ElFormItem>
            </ElCol>
            <ElCol :span="12">
                ...
                <ElRow :gutter="16">
                  <ElCol :span="12">
                    <ElFormItem prop="sub_table_name">
                      <template #label>
                        子表表名
                        <ElTooltip content="数据库中已存在的物理表名，例如 gen_order_item" placement="top">
                          <ElIcon><QuestionFilled /></ElIcon>
                        </ElTooltip>
                      </template>
                      <ElInput
...
const frontendViewDirPreview = computed(() => {
  const pkg = effectivePackageName.value
  const mod = (props.info.module_name || '').trim()
  if (!pkg || !mod) return 'web/src/views/<module_xxx>/<module>/'
  return `web/src/views/${pkg}/${mod}/`
})

const frontendApiFilePreview = computed(() => {
  const pkg = effectivePackageName.value
  const mod = (props.info.module_name || '').trim()
  if (!pkg || !mod) return 'web/src/api/<module_xxx>/<module>.ts'
  return `web/src/api/${pkg}/${mod}.ts`
})
...
watch(
  ...
  () => {
    ...
    const next = mod ? (mod.startsWith('module_') ? mod : `module_${mod}`) : slug ? `module_${slug}` : ''
