  ...
  <FaDrawer
    ...
    <div class="gencode-drawer-step-wrap mt-4">
      ...
      <div v-show="activeStep === 1">
        <GenColumnsStep :info="info" :dict-options="dictOptions" :loading="loading" :bulk-set="bulkSet" />
      </div>
    ...
    <template #footer>
      ...
      <ElButton v-if="activeStep < 2" type="info" :icon="Finished" :loading="loading" @click="emit('save')">
        保存
      </ElButton>
      <ElButton v-if="activeStep !== 0" type="success" :icon="Back" @click="emit('prev-step')">上一步</ElButton>
      <ElButton v-if="activeStep !== 2" type="primary" :loading="nextStepLoading" @click="emit('next-step')">
        {{ nextStepButtonLabel }}
        <ElIcon class="el-icon--right"><Right /></ElIcon>
      </ElButton>
      <ElTooltip v-if="activeStep === 2" content="打包为 ZIP 下载到本机，不会在服务器创建菜单或文件" placement="top">
        <ElButton type="warning" :icon="Download" :loading="loading" @click="emit('gen-download')">下载代码</ElButton>
      </ElTooltip>
      <ElTooltip
        v-if="activeStep === 2"
        content="在服务端项目目录生成源码，并自动创建目录/菜单/按钮（若名称未冲突）"
        placement="top">
        <ElButton type="primary" :icon="FolderOpened" :loading="loading" @click="emit('gen-write')">写入本地</ElButton>
      </ElTooltip>
    </template>
  </FaDrawer>
</template>
...
const nextStepButtonLabel = computed(() => (props.activeStep === 0 ? '下一步：字段配置' : '下一步：预览代码'))
