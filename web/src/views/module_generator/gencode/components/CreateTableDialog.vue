  ...
  <FaDialog
    ...
    <ElAlert type="info" :closable="false" show-icon class="mb-3 !items-start">
      <template #title>
        <span class="text-sm leading-relaxed">
          <template v-if="editMode === 'visual'">选好数据库与单表/主子表，填好表名后点右下角「创建表」即可。</template>
    ...
    <div v-show="editMode === 'sql'" class="sql-pane">
      ...
      <ElScrollbar max-height="min(52vh, 420px)" class="sql-editor-scroll">
        ...
        <Codemirror ref="sqlRef" v-model:value="sqlText" :options="sqlOptions" border height="360px" width="100%" />
      </ElScrollbar>
    </div>
    ...
    <div v-show="editMode === 'visual'" class="visual-pane">
      <ElScrollbar max-height="min(58vh, 520px)" class="visual-pane-scroll">
        <div class="visual-structure max-w-3xl">
          ...
          <ElDescriptions :column="1" border size="small" class="visual-desc mt-3">
            ...
            <ElDescriptionsItem label-class-name="visual-desc-label">
              ...
              <ElInput v-model="visual.mainTableName" placeholder="英文表名，如 gen_demo_order" clearable />
            </ElDescriptionsItem>
          ...
          <ElDescriptions v-if="visual.subEnabled" :column="1" border size="small" class="visual-desc mt-3">
            <template #title>
            ...
            <ElDescriptionsItem label-class-name="visual-desc-label">
              ...
              <ElInput v-model="visual.subTableName" placeholder="如 gen_demo_order_item" clearable />
            </ElDescriptionsItem>
            <ElDescriptionsItem label="说明" label-class-name="visual-desc-label">
            ...
            <ElDescriptionsItem label="外键列" label-class-name="visual-desc-label">
              <ElInput v-model="visual.fkColumn" placeholder="子表里指向主表的那列，如 order_id" clearable />
            </ElDescriptionsItem>
        ...
        <p class="mb-1 mt-2 text-xs text-[var(--el-text-color-secondary)]">将要执行的 SQL（随上面表格自动更新）</p>
        <ElInput
...
import { getExampleFromPresetMasterSub, getExampleFromPresetSingle } from '../utils/createTableSqlExamples'
...
watch(
  ...
  (_d, prev) => {
    ...
    visual.value = visual.value.subEnabled ? applySubColumns(visualPresetMasterSub(d)) : visualPresetSingle(d)
  }
)

function applyVisualPreset(kind: 'single' | 'masterSub') {
  const d = visual.value.dialect
  visual.value = kind === 'single' ? visualPresetSingle(d) : applySubColumns(visualPresetMasterSub(d))
}
...
function loadPresetSql(kind: 'single' | 'masterSub', dialect: SqlDialect) {
  sqlText.value = kind === 'single' ? getExampleFromPresetSingle(dialect) : getExampleFromPresetMasterSub(dialect)
}
...
function handleConfirm() {
  ...
  const sql = editMode.value === 'sql' ? sqlText.value.trim() : buildSqlFromVisual(applySubColumns(visual.value)).trim()
  if (!sql) {
