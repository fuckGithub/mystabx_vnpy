  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        notes: ['锁屏全屏页；背景随明暗主题切换（`@imgs/lock` → `src/assets/images/lock`）；顶栏用户菜单进入。'],
      },
      {
        anchor: 'layout-user',
        title: '用户菜单',
    ...
    pages: [
      {
        ...
        notes: ['静态路由 `/401`；页面含返回/登录入口（见模板）。', '完整性：守卫或 axios 拦截跳转至此页时按钮可用。'],
      },
      {
        anchor: 'page-403',
        title: '403',
        path: 'exception/403/index.vue',
        notes: ['静态路由 `/403`；无权限提示与返回首页等操作。', '完整性：从路由 meta 或业务手动跳转后页面可退出。'],
      },
      {
        anchor: 'page-404',
        title: '404',
      ...
      },
      {
        ...
        notes: ['静态路由 `/500`；服务端错误提示页。', '完整性：错误文案、返回或重试按钮（若有）可点击。'],
      },
    ],
  },
  {
