...
<template>
  <ElDrawer
    ...
    <div v-loading="loading" class="article-detail-drawer-inner">
      ...
      <div v-else-if="articleHtml" class="markdown-body article-detail-markdown" v-highlight v-html="articleHtml" />
    </div>
    <template #footer>
      <div class="flex flex-wrap justify-end gap-2">
        ...
        <ElButton v-if="articleId != null" v-auth="'edit'" type="primary" @click="onEditClick">编辑</ElButton>
      </div>
    </template>
  </ElDrawer>
</template>
...
const fetchDetail = async () => {
  ...
  try {
    const { data } = await axios.get<ArticleResponse>('https://www.qiniu.lingchen.kim/blog_detail.json')
