    ...
    <div class="article-publish-drawer-body">
      <template v-if="resultPhase === 'form'">
        <div>
          <ElRow :gutter="10">
            <ElCol :span="18">
              <ElInput v-model.trim="articleName" placeholder="请输入文章标题（最多100个字符）" maxlength="100" />
            </ElCol>
            <ElCol :span="6">
      ...
      <FaResultPage
        ...
        <template #buttons>
          ...
          <ElButton v-if="pageMode === PageModeEnum.Add" v-ripple @click="publishAgain">新建一篇</ElButton>
        </template>
      </FaResultPage>
...
const drawerTitle = computed(() => (pageMode.value === PageModeEnum.Edit ? '编辑文章' : '新增文章'))

const successTitle = computed(() => (pageMode.value === PageModeEnum.Edit ? '保存成功' : '发布成功'))
...
const getArticleDetail = async () => {
  try {
    const { data } = await axios.get<ArticleDetailResponse>('https://www.qiniu.lingchen.kim/blog_list.json')
