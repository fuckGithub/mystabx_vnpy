...
<template>
  <FaDrawer v-model="drawerVisible" :title="'【' + dictLabel + '】字典数据'" direction="rtl" :size="drawerSize">
    <div class="drawer-content">
      ...
      <ElCard class="fa-table-card drawer-table-card" :style="{ 'margin-top': showSearchBar ? '12px' : '0' }">
        <FaTableHeader
      ...
      <FaDialog
        ...
        <template v-else>
          <ElScrollbar max-height="70vh" :view-style="{ overflowX: 'hidden' }">
            <FaForm
              ...
              <template #css_class>
                <ElSelect
                  ...
                  <ElOption value="primary" label="主要(primary)">
                    <span class="tag-option-preview" :style="getTagPreviewStyle('primary')">主要(primary)</span>
                  </ElOption>
                  <ElOption value="success" label="成功(success)">
                    <span class="tag-option-preview" :style="getTagPreviewStyle('success')">成功(success)</span>
                  </ElOption>
                  <ElOption value="warning" label="警告(warning)">
                    <span class="tag-option-preview" :style="getTagPreviewStyle('warning')">警告(warning)</span>
                  </ElOption>
                  <ElOption value="danger" label="危险(danger)">
                    <span class="tag-option-preview" :style="getTagPreviewStyle('danger')">危险(danger)</span>
                  </ElOption>
                  <ElOption value="info" label="信息(info)">
                    <span class="tag-option-preview" :style="getTagPreviewStyle('info')">信息(info)</span>
                  </ElOption>
                </ElSelect>
              </template>
              <template #list_class>
              ...
              <template #status>
                <ElSwitch v-model="formData.status" inline-prompt :active-value="'0'" :inactive-value="'1'" />
              </template>
            </FaForm>
          </ElScrollbar>
        </template>
        ...
        <template #footer>
          <div class="dialog-footer" style="padding-right: var(--el-dialog-padding-primary)">
            ...
            <ElButton v-if="dialogVisible.type !== 'detail'" type="primary" @click="handleSubmit">确定</ElButton>
...
import DictAPI, { type DictDataForm, type DictDataPageQuery, type DictDataTable } from '@/api/module_system/dict'
...
async function fetchDictDataListMerged(params: Record<string, unknown>) {
  const q: DictDataPageQuery = {
    ...
    status:
      params.status !== undefined && params.status !== null && params.status !== '' ? String(params.status) : undefined,
    created_time: Array.isArray(params.created_time) ? (params.created_time as string[]) : undefined,
    updated_time: Array.isArray(params.updated_time) ? (params.updated_time as string[]) : undefined,
  }
    ...
    columnsFactory: (): ColumnOption<DictDataTable>[] => [
      ...
      { prop: 'dict_label', label: '标签', minWidth: 150, showOverflowTooltip: true },
      {
        ...
        formatter: (row: DictDataTable) => {
          ...
          const cfg = ok ? { type: 'success' as const, text: '启用' } : { type: 'danger' as const, text: '停用' }
      ...
      { prop: 'dict_sort', label: '排序', width: 72 },
      {
        ...
        width: 100,
        formatter: (row: DictDataTable) =>
          h(ElTag, { type: row.is_default ? 'success' : 'danger' }, () => (row.is_default ? '是' : '否')),
      },
