...
import { ElMessage, ElMessageBox, ElTag, ElTooltip, ElDropdown, ElDropdownMenu, ElDropdownItem } from 'element-plus'
...
function formatTenantOperationCell(row: TenantTable, ctx: Parameters<typeof buildTenantRowActions>[1]) {
  ...
  if (overflow.length === 0) {
    return h('div', { class: 'inline-flex flex-wrap items-center justify-end gap-1 tenant-table-actions' }, inlineNodes)
  }
  ...
  return h('div', { class: 'inline-flex flex-wrap items-center justify-end gap-1 tenant-table-actions' }, [
    ...inlineNodes,
    dropdown,
  ])
}
...
const validateTimeRange = (rule: unknown, value: unknown, callback: (e?: Error) => void) => {
  if (formData.value.start_time && formData.value.end_time && formData.value.start_time > formData.value.end_time) {
