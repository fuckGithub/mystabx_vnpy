...
<template>
  <div class="fa-full-height user-manage-page">
    <div class="user-manage-body box-border flex gap-4 h-full max-md:block max-md:gap-0 max-md:h-auto">
      <div class="user-dept-panel flex-shrink-0 w-58 h-full max-md:w-full max-md:h-auto max-md:mb-5">
        <ElCard class="tree-card fa-card-xs flex flex-col h-full mt-0" shadow="hover">
          ...
          <ElScrollbar class="dept-tree-scroll min-h-0 flex-1">
            <DeptTree v-model="deptFilterId" class="dept-tree-inner" @node-click="handleDeptNodeClick" />
          </ElScrollbar>
        </ElCard>
      </div>
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <ElDescriptions :column="2" border>
          ...
          <ElDescriptionsItem label="岗位" :span="2">
            {{ detailFormData.positions ? detailFormData.positions.map((item) => item.name).join('、') : '' }}
          </ElDescriptionsItem>
      ...
      <template v-else>
        <ElForm
          ...
          <ElFormItem label="账号" prop="username">
            <ElInput v-model="formData.username" :disabled="!!formData.id" placeholder="请输入账号" />
          </ElFormItem>
          ...
          <ElFormItem v-if="!formData.id" label="密码" prop="password">
            <ElInput v-model="formData.password" placeholder="请输入密码" type="password" show-password clearable />
          </ElFormItem>
...
function buildUserReplaceParams(u: UserSearchForm): Record<string, unknown> {
  return {
    ...
    created_time: Array.isArray(u.created_time) && u.created_time.length === 2 ? u.created_time : undefined,
  }
}
...
const exportQueryParams = computed(() => {
  ...
  delete sp.page_no
  delete sp.page_size
  if (deptFilterId.value !== undefined && deptFilterId.value !== null && deptFilterId.value !== '') {
...
async function handleOpenDialog(type: 'create' | 'update' | 'detail', id?: number) {
  ...
  if (id) {
    ...
    if (type === 'detail') {
      ...
      formData.value.position_ids = (response.data.data.positions || []).map((item) => item.id as number)
    }
  } else {
