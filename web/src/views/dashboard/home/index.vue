    ...
    <ElCard shadow="hover">
      <div class="flex flex-wrap items-center gap-y-3">
        ...
        <div class="flex min-w-0 flex-1 items-center">
          ...
          <div>
            <div class="workplace-hero__greeting">{{ timefix }}{{ currentUser.name }}，{{ welcome }}</div>
      ...
      <ElCol :span="8" :xs="24" class="mb-3 sm:mb-0">
        <ElCard shadow="hover" class="h-full flex flex-col">
          ...
          <div class="flex items-center justify-between mt-2 flex-1">
            ...
            <FaSvgIcon :icon="resolveIconForFaSvgIcon('people')" class="size-8 animate-[pulse_2s_infinite]" />
          </div>
    ...
    <div class="mt-4 workplace-banners-showcase">
      ...
      <ElRow :gutter="20">
        <ElCol :xs="24" :sm="12" :md="12" class="mb-5">
          <FaBasicBanner title="数据中心运行状态" subtitle="系统访问量同比增长 23%，所有服务运行稳定，数据监控正常。" />
        </ElCol>
        <ElCol :xs="24" :sm="12" :md="12" class="mb-5">
          <FaBasicBanner
          ...
          <FaBasicBanner
            ...
            <template #subtitle>
              <div class="workplace-banners-showcase__slot-sub">
                <p class="workplace-banners-showcase__slot-sub-p">灵活配置，强大扩展，支持自定义插槽内容</p>
              </div>
            </template>
      ...
      <ElRow :gutter="20">
        <ElCol :xs="24" :sm="12" :md="12" :lg="6" class="mb-5">
          <FaCardBanner title="系统运行正常" description="所有核心服务运行稳定，响应时间在正常范围内。" />
        </ElCol>
        <ElCol :xs="24" :sm="12" :md="12" :lg="6" class="mb-5">
          <FaCardBanner
            :image="bannerIcon2"
    ...
    <div class="mt-4 workplace-ops-stack">
      <ElRow :gutter="16" class="workplace-ops-row">
        <ElCol :xs="24" class="workplace-ops-col workplace-ops-col--calendar">
          <ElCard shadow="hover" class="workplace-calendar-card workplace-surface workplace-ops-card">
            <template #header>
              <div class="workplace-section-card__head">
                <div>
                  <span class="workplace-panel-title">日程日历</span>
                  <p class="workplace-section-sub workplace-section-sub--inline">点击日期添加或编辑（本地演示）</p>
                </div>
              </div>
            </template>
    ...
    series: [
      {
        name: '浏览量(PV)',
        type: 'line',
        data: Array.from({ length: visitTrendDateRange.value }, () => Math.floor(Math.random() * 500) + 100),
        areaStyle: {
      ...
      },
      {
        name: '访客数(UV)',
        type: 'line',
        data: Array.from({ length: visitTrendDateRange.value }, () => Math.floor(Math.random() * 200) + 50),
        areaStyle: {
...
function getQuickLinkStableIndex(item: QuickLink): number {
  ...
  const i = list.findIndex((l) => (l.id != null && l.id !== '' ? l.id === item.id : l.href === item.href))
  return i >= 0 ? i : 0
}
