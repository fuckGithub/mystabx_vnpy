        ...
        <ElScrollbar max-height="70vh" :view-style="{ overflowX: 'hidden' }">
          <ElDescriptions :column="2" border>
            ...
            <ElDescriptionsItem label="元数据" :span="2">
              <pre v-if="detailFormData.metadata">{{ JSON.stringify(detailFormData.metadata, null, 2) }}</pre>
              <span v-else>无</span>
            </ElDescriptionsItem>
          </ElDescriptions>
...
const selectedIds = computed(() => selectedRows.value.map((r) => r.id).filter((id): id is string => Boolean(id)))
