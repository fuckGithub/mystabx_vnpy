...
<template>
  <div class="fa-full-height job-page flex flex-col min-h-0">
    ...
    <ElCard class="flex flex-1 min-h-0 flex-col fa-table-card" :style="{ 'margin-top': showSearchBar ? '12px' : '0' }">
      <FaTableHeader
      ...
      <div v-loading="jobLoading" class="job-cards-container min-h-0 flex-1 overflow-auto">
        <ElEmpty v-if="!jobLoading && (!jobList || jobList.length === 0)" :image-size="80" description="暂无数据" />
        <ElRow v-else :gutter="16">
          <ElCol v-for="(job, index) in jobList" :key="job.id" :xs="24" :sm="12" :md="8" :lg="6" class="job-card-col">
        ...
        <ElCard class="fa-table-card log-table-card mt-3 flex flex-1 min-h-0 flex-col">
          ...
          <FaTable
            ...
            <template #log_job_state="{ row }">
              <ElButton v-if="row.job_state" type="primary" size="small" link @click="handleViewJobState(row)">
                查看
              </ElButton>
              <span v-else>-</span>
            </template>
  ...
  core: {
    ...
    columnsFactory: (): ColumnOption<JobLogTable>[] => [
      ...
      },
      {
        ...
        label: '触发方式',
        minWidth: 120,
        formatter: (row) => h(ElTag, { size: 'small' }, () => getTriggerTypeLabel(row.trigger_type)),
      },
      {
        prop: 'status',
        label: '状态',
        minWidth: 80,
        formatter: (row) =>
          h(ElTag, { type: getLogStatusType(row.status), size: 'small' }, () => getLogStatusLabel(row.status)),
      },
      {
        prop: 'next_run_time',
        label: '下次执行时间',
