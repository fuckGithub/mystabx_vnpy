...
function normalizePageResultLike(raw: unknown): PageResult<unknown> | null {
  ...
  const page_no = typeof pageNoRaw === 'number' ? pageNoRaw : typeof pageNoRaw === 'string' ? Number(pageNoRaw) : NaN
  if (!Number.isFinite(page_no)) return null

  const pageSizeRaw = o.page_size ?? o.size ?? o.limit ?? o.pageSize
  const page_size =
    typeof pageSizeRaw === 'number' ? pageSizeRaw : typeof pageSizeRaw === 'string' ? Number(pageSizeRaw) : NaN
...
function unwrapAxiosResponseBody(response: unknown): unknown {
  ...
  if ('data' in r && 'status' in r && typeof r.status === 'number' && 'config' in r && typeof r.config === 'object') {
    return r.data
  }
...
export const updatePaginationFromResponse = <T>(pagination: { total: number }, response: ApiResponse<T>): void => {
...
export function renderTableOperationCell(
  ...
  const wrapperClass = options?.wrapperClass ?? 'inline-flex flex-wrap items-center justify-end gap-1'
  ...
  const inlineNodes = inline.map((a) =>
    h(ElTooltip, { content: a.label, placement: 'top' }, () =>
      h('span', { class: a.disabled ? 'inline-flex opacity-40 pointer-events-none' : 'inline-flex' }, [
        h(ArtButtonTable, {
          type: a.artType,
