...
interface FaExportDialogProps {
  /** 内容配置 */
  contentConfig: Pick<IContentConfig, 'permPrefix' | 'cols' | 'exportsAction' | 'exportsBlobAction'>
  /** 查询参数 */
  queryParams?: IObject
  /** 页面数据 */
  pageData?: IObject[]
...
const cols = computed(() =>
  props.contentConfig.cols.map((col) => {
    ...
    if (col.type === 'selection' && col.reserveSelection === undefined && col['reserve-selection'] === undefined) {
      // 配合表格row-key实现跨页多选
