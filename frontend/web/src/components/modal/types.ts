import type { DialogProps, DrawerProps, FormItemRule, FormRules, PaginationProps } from 'element-plus'
...
type OtherComponent = 'text' | 'radio' | 'checkbox' | 'switch' | 'rate' | 'slider' | 'icon-select' | 'custom'
...
export interface IContentConfig<T = any> {
  ...
  pagination?:
    | boolean
    | Partial<Omit<PaginationProps, 'v-model:page-size' | 'v-model:current-page' | 'total' | 'currentPage'>>
  ...
  modifyAction?: (data: { [key: string]: any; field: string; value: boolean | string | number }) => Promise<any>
  ...
  cols: Array<{
    ...
    show?: boolean
    /** 模板类型 */
    templet?: 'image' | 'list' | 'url' | 'switch' | 'input' | 'price' | 'percent' | 'icon' | 'date' | 'tool' | 'custom'
    /** image模板相关参数 */
    imageWidth?: number
    /** image模板相关参数 */
    imageHeight?: number
