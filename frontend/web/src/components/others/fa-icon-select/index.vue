        ...
        <div @click="popoverVisible = !popoverVisible">
          <slot>
            <ElInput v-model="selectedIcon" readonly placeholder="点击选择图标" class="reference">
              ...
              <template #suffix>
                <!-- 清空按钮 -->
                <ElIcon v-if="selectedIcon" style="margin-right: 8px" @click.stop="clearSelectedIcon">
                  <CircleClose />
                </ElIcon>
...
function filterIcons() {
  if (activeTab.value === 'svg') {
    ...
    filteredElementIcons.value = filterText.value
      ? elementIcons.value.filter((icon) => icon.toLowerCase().includes(filterText.value.toLowerCase()))
      : elementIcons.value
  }
}
