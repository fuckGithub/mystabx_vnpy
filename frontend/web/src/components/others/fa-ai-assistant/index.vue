<template>
  ...
  <div class="ai-assistant">
    ...
    <!-- 收缩态：贴边小标签，避免遮挡表单控件 -->
    <div v-if="!dialogVisible && fabCollapsed" class="ai-fab-tab" :style="fabStyle" @click="fabCollapsed = false">
      AI
    </div>
    ...
    <ElDialog
      ...
      <div class="quick-commands">
        ...
        <ElTag v-for="example in examples" :key="example" class="command-tag" @click="command = example">
          {{ example }}
        </ElTag>
      </div>
...
const updateFabPosition = () => {
  ...
  const intersects = (a: { left: number; right: number; top: number; bottom: number }, b: DOMRect) => {
...
// 快捷命令示例
const examples = ['跳转到用户管理', '打开角色管理页面', '修改test用户的姓名为测试人员', '获取姓名为张三的用户信息']
...
const extractKeywordFromCommand = (cmd: string): string => {
  ...
  const patterns = [
    ...
    new RegExp(`(?:姓名为|名字叫|叫做|名称为|名是|为)([^\\s，。]+?)(?:的)?(?:${keywordsPattern})?`, 'i'),
  ...
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      ...
      if (extracted && !allKeywords.some((type) => extracted.toLowerCase().includes(type.toLowerCase()))) {
        return extracted
      }
    }
  }
...
const tryDirectNavigate = (rawCommand: string): AiResponse | null => {
  ...
  const operationIntents = ['修改', '更新', '变更', '删除', '添加', '创建', '设置', '获取', '查询', '搜索']
...
const parseAction = (result: any, rawCommand: string): AiAction | null => {
  ...
  if (primaryCall && functionName) {
    ...
    if (isQueryFunction) {
      ...
      const keywords = typeof args.keywords === 'string' && args.keywords.trim().length > 0 ? args.keywords : keyword
