  ...
  () => {
    ...
    const validateWorktabs = (routerInstance: Router): void => {
      try {
        ...
        // 过滤出有效的标签页
        const validTabs = opened.value.filter((tab) => isTabRouteValid(tab) && !isLoginWorktab(tab))
        ...
        const isCurrentValid = current.value && isTabRouteValid(current.value) && !isLoginWorktab(current.value)
