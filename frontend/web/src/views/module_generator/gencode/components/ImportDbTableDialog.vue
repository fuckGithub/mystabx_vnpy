<template>
  <FaDialog ref="dialogRef" v-model="open" title="导入表" width="min(960px, 96vw)" append-to-body>
    <ElForm ref="importQueryRef" :model="query" :inline="true">
      ...
      <ElFormItem>
        <ElButton v-hasPerm="['module_generator:dblist:query']" type="primary" icon="Search" @click="emit('query')">
          搜索
        </ElButton>
        <ElButton v-hasPerm="['module_generator:dblist:query']" icon="Refresh" @click="emit('reset')">重置</ElButton>
      </ElFormItem>
    </ElForm>
    <ElRow>
      <ElTable
        ...
        <ElTableColumn prop="database_name" label="数据库名称" :show-overflow-tooltip="true"></ElTableColumn>
        <ElTableColumn prop="table_name" label="表名称" :show-overflow-tooltip="true"></ElTableColumn>
        <ElTableColumn prop="table_comment" label="表描述" :show-overflow-tooltip="true"></ElTableColumn>
