...
export function visualPresetSingle(dialect: SqlDialect): VisualBuildState {
  ...
  const business: ColDef[] = [{ name: 'name', type: 'varchar(64)', nullable: true, isPk: false, comment: '名称' }]
  return {
    dialect,
 ...
 */
export function mergeGenTableLinkIntoVisual(link: GenTableCreateLink, dialect: SqlDialect): VisualBuildState {
  ...
  const base: VisualBuildState = hasSubPair ? visualPresetMasterSub(dialect) : visualPresetSingle(dialect)
