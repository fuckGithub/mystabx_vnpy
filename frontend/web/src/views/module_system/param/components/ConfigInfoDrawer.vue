<!-- 参数配置 -->
<template>
  <FaDrawer v-model="drawerVisible" title="配置中心" :size="drawerSize" destroy-on-close @close="onDrawerClosed">
              ...
              <div class="space-y-2">
                <div v-for="listItem in apiWhitelistItems" :key="listItem.id" class="flex items-center gap-2">
                  <ElInput
                    ...
                    clearable
                    @input="markModified(key)"
                    @blur="validateApiPathOnBlur(listItem.value)" />
                  <ElButton
                    type="danger"
                    icon="minus"
                    circle
                ...
                <ElButton type="primary" icon="plus" size="small" style="margin-top: 10px" @click="addApiWhitelistItem">
                  添加接口路径
                </ElButton>
              ...
              <div class="space-y-2">
                <div v-for="listItem in ipBlacklistItems" :key="listItem.id" class="flex items-center gap-2">
                  <ElInput
                    ...
                    style="flex: 1"
                    @input="markModified(key)"
                    @blur="validateIpOnBlur(listItem.value)" />
                  <ElButton
                    type="danger"
                    icon="minus"
                    circle
                ...
                <ElButton type="primary" icon="plus" size="small" style="margin-top: 10px" @click="addIpBlacklistItem">
                  添加IP地址
                </ElButton>
                <div class="text-xs text-gray-500 mt-2">
            ...
            <ElFormItem :label="item.config_name">
              ...
              <template v-if="key === 'demo_enable'">
                <ElSwitch
                  ...
                  " />
                <div class="text-xs text-gray-500 mt-1">配置说明：启用后系统将进入演示模式，部分功能可能受限。</div>
              </template>
              <!-- IP白名单 -->
...
// 接口路径失焦验证
const validateApiPathOnBlur = (value: string) => {
  if (!isValidApiPath(value) && value.trim()) {
    ElMessage.warning('请输入有效的接口路径格式（以/开头）')
  }
}

// IP 地址失焦验证
const validateIpOnBlur = (value: string) => {
  if (!isValidIp(value) && value.trim()) {
