    ...
    <FaDialog
      ...
      <ElScrollbar max-height="75vh" :view-style="{ overflowX: 'hidden' }">
        <ElDescriptions :column="8" border label-width="200px">
          ...
          <ElDescriptionsItem label="操作账号" :span="2">
            {{ formData.username }}
          </ElDescriptionsItem>
          <ElDescriptionsItem label="手机号" :span="2">
            {{ formData.mobile }}
          </ElDescriptionsItem>
          <ElDescriptionsItem label="登录平台" :span="2">
            <ElTag v-if="formData.login_platform" type="success">
              {{
                { PC: 'PC端', FLUTTER: 'Flutter移动端', UNIAPP: 'UniApp移动端' }[formData.login_platform] ??
                formData.login_platform
              }}
            </ElTag>
            <span v-else>—</span>
          </ElDescriptionsItem>
...
function buildLogReplaceParams(p: LogSearchForm): Record<string, unknown> {
  return {
    ...
    type: p.type !== undefined && p.type !== null && p.type !== ('' as unknown as number) ? Number(p.type) : undefined,
    response_code:
      p.response_code !== undefined && p.response_code !== null && p.response_code !== ('' as unknown as number)
        ? Number(p.response_code)
        : undefined,
    ...
    mobile: p.mobile,
    login_platform:
      p.login_platform !== undefined && p.login_platform !== null && p.login_platform !== ''
        ? p.login_platform
        : undefined,
  }
}
...
const logSearchItems = computed<SearchFormItem[]>(() => [
  ...
  },
  {
    ...
    props: {
      ...
      options: [
        { label: 'PC端', value: 'PC' },
        { label: 'Flutter移动端', value: 'FLUTTER' },
        { label: 'UniApp移动端', value: 'UNIAPP' },
      ],
      clearable: true,
    },
    span: 6,
    ...
    columnsFactory: (): ColumnOption<LogTable>[] => [
      { type: 'selection', width: 48, fixed: 'left' },
      {
        ...
        formatter: (row: LogTable) =>
          h(ElTag, { type: row.type === 1 ? 'success' : 'primary' }, () => (row.type === 1 ? '登录日志' : '操作日志')),
      },
      { prop: 'username', label: '操作账号', minWidth: 120, showOverflowTooltip: true },
      { prop: 'mobile', label: '手机号', minWidth: 130 },
      {
        ...
        minWidth: 120,
        formatter: (row: LogTable) => {
          const map: Record<string, string> = { PC: 'PC端', FLUTTER: 'Flutter移动端', UNIAPP: 'UniApp移动端' }
          return h(ElTag, { type: 'success' }, () => map[row.login_platform ?? ''] ?? row.login_platform ?? '—')
        },
      },
      ...
      },
      {
        ...
        minWidth: 100,
        formatter: (row: LogTable) =>
          h(ElTag, { type: getStatusCodeType(row.response_code) }, () => String(row.response_code ?? '')),
      },
      {
        prop: 'request_ip',
        label: '请求IP',
