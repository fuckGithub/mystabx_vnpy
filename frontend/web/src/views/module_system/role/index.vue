        ...
        <ElScrollbar max-height="75vh" :view-style="{ overflowX: 'hidden' }">
          <ElDescriptions :column="4" border>
            ...
            <ElDescriptionsItem label="数据权限" :span="2">
              ...
              <ElTag v-else-if="detailFormData.data_scope === 3" type="warning">本部门及以下数据权限</ElTag>
...
import RoleAPI, { type RoleForm, type RoleTable, type TablePageQuery } from '@/api/module_system/role'
...
function deptsCell(row: RoleTable) {
  ...
  if (list.length > 3) {
    tags.push(h(ElTag, { type: 'info', style: { marginBottom: '4px' } }, () => `+${list.length - 3}`))
  }
