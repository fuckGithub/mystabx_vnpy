        ...
        <div
          ...
          <footer
            ...
            <div class="login-footer-text text-sm">
              ...
              <span v-if="configStore.configData?.sys_keep_record?.config_value" class="login-page-footer__record">
...
import AuthAPI, { type CaptchaInfo, type LoginFormData, type OAuthProvider } from '@/api/module_system/auth'
...
const panelTitle = computed(() => {
  ...
  if (authPanel.value === 'login' && (loginFlowMode.value === 'mobile' || loginFlowMode.value === 'qr')) {
...
const dragVerifyTextColor = computed(() => (isDark.value ? 'rgba(255, 255, 255, 0.45)' : 'var(--fa-gray-700)'))
const formKey = ref(0)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 登录成功后先等动态路由注册就绪，再执行跳转。
    // 否则 router.replace 触发守卫时，守卫需在初始化里异步拉菜单 +
    // addRoute（耗时），期间若并发导航命中在途初始化，将由守卫等待后重新导航；
    // 提前等待可使本次 replace 成为直通导航（守卫命中 shouldInitRoutes=false）。
