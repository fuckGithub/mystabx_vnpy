            ...
            <ElCard shadow="never" class="app-card" :class="{ 'card-disabled': !app.status }">
              <template #header>
                <div class="app-info-header">
                  ...
                  <ElTag :type="app.status ? 'success' : 'info'" size="small" effect="plain" class="app-status">
                    {{ app.status ? '启用' : '停用' }}
                  </ElTag>
                </div>
              </template>
              ...
              <template #footer>
                <div class="card-footer-row">
                  <div class="card-meta">{{ app.created_by?.name || '—' }} · {{ formatTime(app.created_time) }}</div>
...
import ApplicationAPI, { type ApplicationForm, type ApplicationInfo } from '@/api/module_application/portal'
