          ...
          <div class="history-groups">
            <div v-for="group in groupedSessions" :key="group.title" class="history-group">
              <div class="group-title" @click="toggleGroup(group.title)">
                ...
                <ElIcon class="collapse-icon" :class="{ collapsed: collapsedGroups.has(group.title) }">
                  <ArrowDown />
                </ElIcon>
              </div>
              <div v-show="!collapsedGroups.has(group.title)" class="session-list">
                <div
                  ...
                  <ElDropdown trigger="click" @command="(cmd) => handleSessionCommand(cmd, session)">
...
import AiChatAPI, { ChatSession, SessionGroup, UserInfo } from '@/api/module_ai/chat'
import { ArrowDown, ChatLineRound, MoreFilled, Plus, Search, Setting, User } from '@element-plus/icons-vue'
...
const filteredSessions = computed<ChatSession[]>(() => {
  ...
  return sessions.value.filter((session: ChatSession) => (session.title || '').toLowerCase().includes(query))
})
