...
<template>
  <div class="fa-full-height resource-monitor-page">
    ...
    <ElCard
      ...
      <div class="resource-breadcrumb mb-3 shrink-0 flex flex-wrap items-center gap-2 border-b border-g-200 pb-3">
    ...
    <FaDialog v-model="uploadDialogVisible" title="上传文件" width="500px" @close="handleUploadClose">
      <ElUpload
        ref="uploadRef"
        :auto-upload="false"
        :multiple="true"
    ...
    <FaDialog v-model="createDirDialogVisible" title="新建文件夹" width="400px">
      ...
      <template #footer>
        ...
        <ElButton v-hasPerm="['module_monitor:resource:create_dir']" type="primary" @click="handleCreateDirConfirm">
          确定
        </ElButton>
      </template>
    </FaDialog>
    ...
    <FaDialog v-model="renameDialogVisible" title="重命名" width="400px">
      <ElForm :model="renameForm" label-width="80px">
        <ElFormItem label="新名称" required>
          <ElInput v-model="renameForm.new_name" placeholder="请输入新名称" @keyup.enter="handleRenameConfirm" />
        </ElFormItem>
      </ElForm>
      <template #footer>
        <ElButton @click="renameDialogVisible = false">取消</ElButton>
        <ElButton v-hasPerm="['module_monitor:resource:rename']" type="primary" @click="handleRenameConfirm">
          确定
        </ElButton>
      </template>
    </FaDialog>
...
import { Delete, Document, Folder, FolderAdd, Plus, QuestionFilled, UploadFilled } from '@element-plus/icons-vue'
        ...
        formatter: (row: ResourceItem) =>
          h('div', { class: 'file-name flex items-center gap-2' }, [
            ...
            h(
              'span',
              {
                class: 'file-name-clickable cursor-pointer text-theme hover:text-theme/80 hover:underline',
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 个文件吗？`, '确认删除', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
