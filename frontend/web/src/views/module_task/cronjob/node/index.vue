...
<template>
  <div class="fa-full-height">
          ...
          <ElScrollbar style="height: 100%">
            <FaForm
              ...
              <template #args>
                <div class="dynamic-params">
                  <div v-for="(_item, index) in argsList" :key="index" class="param-item">
                    ...
                    <ElButton type="danger" icon="Delete" circle @click="argsList.splice(index, 1)" />
                  </div>
                  <ElButton type="primary" icon="Plus" @click="argsList.push('')">添加位置参数</ElButton>
                </div>
              </template>
              <template #kwargs>
                <div class="dynamic-params">
                  <div v-for="(item, index) in kwargsList" :key="index" class="param-item">
                    ...
                    <ElButton type="danger" icon="Delete" circle @click="kwargsList.splice(index, 1)" />
                  </div>
                  <ElButton type="primary" icon="Plus" @click="kwargsList.push({ key: '', value: '' })">
                    添加关键词参数
                  </ElButton>
                </div>
              </template>
              ...
              <template #max_instances>
                <ElInputNumber v-model="formData.max_instances" controls-position="right" :min="1" :max="10" />
              </template>
            </FaForm>
          </ElScrollbar>
        </ElSplitterPanel>
    ...
    <FaDialog v-model="executeDialogVisible" title="调试节点" width="700px" @close="handleCloseExecuteDialog">
      <FaForm
        ...
        <template #trigger_args>
          ...
          <template v-else-if="executeFormData.trigger === 'interval'">
            <ElPopover :visible="openInterval" width="600px" trigger="click" :persistent="false" placement="auto-end">
              <template #reference>
                <ElInput
                  v-model="executeFormData.trigger_args"
        ...
        const submitData = {
          ...
          kwargs:
            kwargsList.value.filter((v) => v.key.trim()).length > 0
              ? JSON.stringify(
                  Object.fromEntries(kwargsList.value.filter((v) => v.key.trim()).map((v) => [v.key, v.value]))
                )
              : undefined,
        }
        if (id) {
