        ...
        <ElScrollbar max-height="70vh" :view-style="{ overflowX: 'hidden' }">
          <ElDescriptions :column="4" border>
            ...
            <ElDescriptionsItem label="元数据" :span="2">
              <JsonPretty v-if="detailFormData.i != null" :value="detailFormData.i" height="140px" />
            </ElDescriptionsItem>
        ...
        <ElScrollbar max-height="70vh" :view-style="{ overflowX: 'hidden' }">
          <ElForm
            ...
            <ElFormItem label="浮点数" prop="c">
              <ElInputNumber v-model="formData.c" placeholder="请输入浮点数" :step="0.01" :precision="2" />
            </ElFormItem>
            ...
            <ElFormItem label="时间" prop="f">
              <ElTimePicker v-model="formData.f" placeholder="请选择时间" style="width: 100%" value-format="HH:mm:ss" />
            </ElFormItem>
            ...
            <ElFormItem label="元数据" prop="i">
              <div class="flex flex-col gap-2">
                <div v-for="(item, index) in metadataList" :key="index" class="flex items-center gap-2">
                  <ElInput v-model="item.key" placeholder="键" />
                  <ElInput v-model="item.value" placeholder="值" />
                  <ElButton type="primary" icon="Plus" circle @click="metadataList.push({ key: '', value: '' })" />
                  <ElButton type="danger" icon="Delete" circle @click="metadataList.splice(index, 1)" />
                </div>
                <ElButton
...
import DemoAPI, { type DemoForm, type DemoPageQuery, type DemoTable } from '@/api/module_example/demo'
    ...
    columnsFactory: (): ColumnOption<DemoTable>[] => [
      ...
      { prop: 'uuid', label: 'UUID', minWidth: 168, showOverflowTooltip: true },
      {
        ...
        formatter: (row: DemoTable) => {
          ...
          const cfg = ok ? { type: 'success' as const, text: '启用' } : { type: 'info' as const, text: '停用' }
      ...
      { prop: 'c', label: '浮点数', minWidth: 88, showOverflowTooltip: true },
      {
        ...
        label: '布尔',
        width: 80,
        formatter: (row: DemoTable) => h(ElTag, { type: row.d ? 'success' : 'danger' }, () => (row.d ? '是' : '否')),
      },
...
const deleteDemoRow = async (row: DemoTable) => {
  if (!row.id) return
  try {
    await ElMessageBox.confirm(`确定删除「${row.name ?? row.id}」吗？此操作不可恢复！`, '删除确认', {
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm(`确定删除选中的 ${ids.length} 条数据吗？此操作不可恢复！`, '批量删除', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
...
async function runBatchStatus(status: string) {
  ...
  try {
    await ElMessageBox.confirm(`确认对选中的 ${ids.length} 条数据${status === '0' ? '启用' : '停用'}？`, '批量设置', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
