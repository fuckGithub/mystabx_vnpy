...
import { TableCache, CacheInvalidationStrategy, type ApiResponse as TableApiResponse } from '@utils/table'
import {
  type TableError,
...
type InferApiResponse<T> = T extends (params: any) => Promise<infer R> ? UnwrapAxiosResponse<R> : never
...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(config: UseTableConfig<TApiFn>) {
   ...
   */
  const fetchData = async (params?: Partial<TParams>, useCache = enableCache): Promise<TableApiResponse<TRecord>> => {
    ...
    try {
      ...
      globalListNetworkInflight.set(dedupeKey, networkPromise as Promise<TableApiResponse<unknown>>)
  ...
  const getDataByPage = async (params?: Partial<TParams>): Promise<TableApiResponse<TRecord> | void> => {
