...
export function useTheme() {
  ...
  const setSystemTheme = (theme: SystemThemeEnum, themeMode?: SystemThemeEnum) => {
    ...
    const currentTheme = AppEnv.systemThemeStyles[theme as keyof SystemThemeTypes]
...
export function initializeTheme() {
  ...
  const applyThemeByMode = () => {
    ...
    // 设置主题 class
    const currentTheme = AppEnv.systemThemeStyles[actualTheme as keyof SystemThemeTypes]
    if (currentTheme) {
      el.setAttribute('class', currentTheme.className)
    }
