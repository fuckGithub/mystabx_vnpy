...
export class ComponentLoader {
  ...
  load(componentPath: string): () => Promise<any> {
    ...
    const normalized = componentPath.startsWith('/') ? componentPath : `/${componentPath.replace(/^\/+/, '')}`
...
export class RouteTransformer {
  ...
  private handleFirstLevelLeaf(converted: ConvertedRoute, route: AppRouteRecord, component: string | undefined): void {
    converted.component = this.shellChild ? this.componentLoader.loadNestedParent() : this.componentLoader.loadLayout()
    converted.name = ''
...
export class RouteRegistry {
  ...
  register(menuList: AppRouteRecord[]): void {
    ...
    menuList.forEach((route, index) => {
      ...
      removeRouteFns.push(this.router.addRoute(ROOT_LAYOUT_ROUTE_NAME, routeConfig as RouteRecordRaw))
    })
