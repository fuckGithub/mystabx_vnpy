...
import { mergeAppRouteRecords, ROUTE_COMPONENT_LAYOUT, ROUTE_COMPONENT_NESTED_PARENT } from './staticRoutes'
...
function normalizeMenuNestedPaths(items: MenuTable[], parentAbsolutePath?: string): MenuTable[] {
  return items.map((node) => {
    ...
    const children = node.children?.length ? normalizeMenuNestedPaths(node.children, canonical) : undefined
...
function normalizeAppRouteChildPaths(routes: AppRouteRecord[], parentAbsolutePath = ''): AppRouteRecord[] {
  return routes.map((route) => {
    ...
    const children = route.children?.length ? normalizeAppRouteChildPaths(route.children, currentAbs) : route.children
...
function mapMenuNode(item: MenuTable, depth = 0): AppRouteRecord {
  ...
  const children = childrenRaw.length ? childrenRaw.map((c) => mapMenuNode(c, depth + 1)) : undefined
