...
async function cleanRouteModules() {
  ...
  try {
    ...
    if (reset === mp) {
      console.log(`     ${icons.warn} ${fmt.dim('MenuProcessor.ts 中未找到 builtinFrontendRoutes，跳过重置')}`)
    } else {
...
async function cleanRouterConstants() {
  console.log(`     ${icons.info} ${fmt.dim('路由常量（ROUTE_*）位于 staticRoutes.ts 顶部，请按需手动修改')}`)
}
