...
const subPagePath: Record<string, string> = {
  settings: '/subPages/settings/index',
  profile: '/subPages/profile/index',
  feedback: '/subPages/feedback/index',
  account: '/subPages/account/index',
  user: '/pages/work/user/index',
  position: '/subPages/position/index',
  role: '/pages/work/role/index',
  parameter: '/subPages/parameter/index',
  dictionary: '/subPages/dictionary/index',
  department: '/subPages/department/index',
  menu: '/subPages/menu/index',
  log: '/subPages/log/index',
}

function navigateTo(name: string) {
  const path = subPagePath[name]
  if (path) {
...
<template>
  <view class="p-3">
      ...
      <wd-row class="flex items-center">
        <wd-col :span="6">
          <wd-badge v-if="true" model-value="99+" @click="handleMessagesClick">
            <wd-avatar size="medium" shape="round" :src="isLogin ? userInfo!.avatar : defaultAvatar" />
          </wd-badge>
        </wd-col>
        <wd-col :span="16">
    ...
    <!-- 退出登录按钮 -->
    <wd-button v-if="isLogin" class="p-3" size="large" block @click="handleLogout">退出登录</wd-button>
  </view>
</template>
