...
<template>
  <view class="min-h-screen bg-gray-100 py-3 dark:bg-[var(--wot-dark-background)]">
    ...
    <view class="mx-3 mb-3">
      <view class="rounded-3 bg-white px-5 py-8 text-center dark:bg-[var(--wot-dark-background2)]">
        ...
        <view class="mb-2 text-6 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">Pinia 状态管理</view>
        <view class="mb-2 text-3.5 text-gray-600 leading-relaxed dark:text-[var(--wot-dark-color2)]">
          Vue 3 的直观状态管理库
        </view>
        <view class="mb-2 text-3 text-gray-500 dark:text-[var(--wot-dark-color2)]">轻量、类型安全、支持组合式API</view>
      ...
      <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
        ...
        <view class="space-y-3">
          <view class="rounded bg-gray-100 p-3 dark:bg-[var(--wot-dark-background3)]">
            <view class="mb-2 text-3 text-gray-700 font-bold dark:text-[var(--wot-dark-color)]">定义 Store:</view>
            <view
              class="text-2.5 text-gray-600 leading-relaxed font-mono dark:text-[var(--wot-dark-color2)]"
              @click="
  ...
  return { count, doubleCount, increment }
          ...
          <view class="rounded bg-gray-100 p-3 dark:bg-[var(--wot-dark-background3)]">
            <view class="mb-2 text-3 text-gray-700 font-bold dark:text-[var(--wot-dark-color)]">使用 Store:</view>
            <view
              class="text-2.5 text-gray-600 leading-relaxed font-mono dark:text-[var(--wot-dark-color2)]"
              @click="
...
console.log(counterStore.count)`)
    ...
    <demo-block title="持久化演示 - 用户信息" transparent>
      <view class="mb-3 border border-green-200 rounded-2 bg-green-50 p-3 dark:border-green-800 dark:bg-green-900/20">
        <view class="mb-1 text-3.5 text-green-700 font-bold dark:text-green-300">✅ 持久化存储</view>
      ...
      <view class="space-y-4">
        <!-- 当前用户信息 -->
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">当前用户信息</view>
          ...
          <view v-else class="text-center text-gray-500 dark:text-[var(--wot-dark-color2)]">暂无用户信息</view>
        </view>

        <!-- 用户信息表单 -->
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">设置用户信息</view>

          <view class="space-y-3">
            <view>
              <view class="mb-1 text-3 text-gray-700 dark:text-[var(--wot-dark-color)]">用户名:</view>
              <wd-input v-model="userForm.name" placeholder="请输入用户名" />
            </view>

            <view>
              <view class="mb-1 text-3 text-gray-700 dark:text-[var(--wot-dark-color)]">邮箱:</view>
              <wd-input v-model="userForm.email" placeholder="请输入邮箱" />
            </view>

            <view class="grid grid-cols-2 gap-2">
              <wd-button type="primary" @click="saveUserInfo">保存信息</wd-button>
              <wd-button type="error" @click="userStore.clearUserInfo">清除信息</wd-button>
            </view>
          </view>
        </view>
      </view>
    ...
    <demo-block title="非持久化演示 - 临时数据" transparent>
      <view
        ...
        <view class="mb-1 text-3.5 text-orange-700 font-bold dark:text-orange-300">⚠️ 临时存储</view>
      ...
      <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
        <view class="mb-3 text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">临时消息</view>
        ...
        <view v-else class="mb-4 text-center text-gray-500 dark:text-[var(--wot-dark-color2)]">暂无临时消息</view>

        <view class="space-y-3">
          <wd-input v-model="tempMessage" placeholder="请输入临时消息" />
          <view class="grid grid-cols-2 gap-2">
            <wd-button type="primary" size="small" @click="setTempMessage">设置消息</wd-button>
            <wd-button type="warning" size="small" @click="tempStore.clearMessage">清除消息</wd-button>
          </view>
        </view>
      </view>
    </demo-block>
    ...
    <demo-block title="持久化实现原理" transparent>
      <view class="space-y-4">
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">持久化插件</view>
  ...
  store.$subscribe(() => {
    uni.setStorageSync(store.$id, store.$state)
            ...
            ">
            <view class="mb-2 text-3 text-gray-700 font-bold dark:text-[var(--wot-dark-color)]">核心实现:</view>
            <view class="text-2.5 text-gray-600 leading-relaxed font-mono dark:text-[var(--wot-dark-color2)]">
              ...
              (excludedIds.includes(store.$id)) return\n &nbsp;&nbsp;// 从本地存储读取数据\n &nbsp;&nbsp;const
              storageState = uni.getStorageSync(store.$id)\n &nbsp;&nbsp;// 监听状态变化并保存\n
              &nbsp;&nbsp;store.$subscribe(() => {\n &nbsp;&nbsp;&nbsp;&nbsp;uni.setStorageSync(store.$id,
              store.$state)\n &nbsp;&nbsp;})\n }
            </view>
          </view>
        </view>
        ...
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">排除列表</view>
          <view class="mb-3 text-3 text-gray-600 leading-relaxed dark:text-[var(--wot-dark-color2)]">
...
persist(context, ['global-register', 'temp'])`)
        ...
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">使用说明</view>
          <view class="text-3 text-gray-600 leading-relaxed space-y-2 dark:text-[var(--wot-dark-color2)]">
            ...
            <view>
              •
              <text class="text-orange-600 font-bold dark:text-orange-400">排除机制</text>
              ：通过 Store ID 控制哪些不需要持久化
            </view>
            <view>
              •
      ...
      <view class="space-y-3">
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 flex items-center">
            ...
            <view class="text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">Store 设计建议</view>
          </view>
          <view class="text-3 text-gray-600 leading-relaxed space-y-2 dark:text-[var(--wot-dark-color2)]">
        ...
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 flex items-center">
            <view class="mr-2 text-6">🔒</view>
            <view class="text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">持久化建议</view>
          </view>
          <view class="text-3 text-gray-600 leading-relaxed space-y-2 dark:text-[var(--wot-dark-color2)]">
        ...
        <view class="rounded-2 bg-white p-4 dark:bg-[var(--wot-dark-background2)]">
          <view class="mb-3 flex items-center">
            <view class="mr-2 text-6">⚡</view>
            <view class="text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">性能优化</view>
          </view>
          <view class="text-3 text-gray-600 leading-relaxed space-y-2 dark:text-[var(--wot-dark-color2)]">
