...
开发前先加载 `.agents/skills/mobile-unibest-skill.md`，获取完整的开发规范（技术栈、项目结构、HTTP 请求封装、状态管理、路由、TabBar、多端适配、工具函数等）。

使用 Wot UI 组件时，推荐同时加载 `.agents/skills/wot-ui-skill.md`，获取官方组件用法、API 查询、主题定制、常见坑位排查等指南。
