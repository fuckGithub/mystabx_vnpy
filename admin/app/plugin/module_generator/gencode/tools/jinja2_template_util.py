...
from app.core.plugin.manifest import PLUGIN_DIR_PREFIX, plugin_name_from_dir
...
class Jinja2TemplateUtil:
    ...
    def prepare_context(cls, gen_table: GenTableOutSchema) -> dict[str, Any]:
        ...
        plugin_name = cls.get_plugin_name(package_name)
        model_paths = cls.get_plugin_model_paths(gen_table)
        ...
        context = {
            ...
            # 插件骨架（A10）：plugin.toml 的 name（目录名去掉 module_ 前缀）与 plugin.py 的模型点位
            "plugin_name": plugin_name,
            "model_paths": model_paths,
            "model_paths_literal": cls.format_python_tuple_literal(model_paths),
    ...
    def get_template_list(cls):
        ...
        """
        templates = [
            # 插件骨架（A10）：输出到插件根 module_xxx/，使生成结果即为合规插件
            "python/plugin.toml.j2",
            "python/plugin.py.j2",
            "python/README.md.j2",
    ...
    @classmethod
    def get_sub_template_list(cls) -> list[str]:
        """子表渲染使用的模板清单：排除只随主表渲染一次的插件骨架模板。
        ...
        - list[str]: 模板路径列表。
        """
        return [t for t in cls.get_template_list() if t not in cls.PLUGIN_ROOT_TEMPLATES]
    ...
    def get_file_name(cls, template: str, gen_table: GenTableOutSchema):
        ...
        template_mapping = {
            # 插件骨架（A10）：一律写到插件根 module_xxx/，与子模块目录无关
            "plugin.toml.j2": f"{backend_base}/plugin.toml",
            "plugin.py.j2": f"{backend_base}/plugin.py",
            "README.md.j2": f"{backend_base}/README.md",
    ...
    @classmethod
    def get_plugin_name(cls, package_name: str | None) -> str:
        """由插件目录名推导插件名（与内核 loader 同一规则）：``module_ai`` → ``ai``。

        参数:
        - package_name (str | None): 插件目录名，如 ``module_ai``。

        返回:
        - str: 插件名；缺 ``module_`` 前缀时原样返回，供 plugin.toml 的 ``name`` 使用。
        """
        name = (package_name or "").strip()
        if not name.startswith(PLUGIN_DIR_PREFIX):
            return name
        try:
            return plugin_name_from_dir(name)
        except ValueError:  # 前缀后为空（裸 module_）
            return name

    @classmethod
    def get_plugin_model_paths(cls, gen_table: GenTableOutSchema) -> list[str]:
        """插件骨架 ``plugin.py`` 的 ``MODEL_PATHS``：模型模块的点分路径列表。

        主表固定为 ``{module_name}.model``；子表与主表同模块时合并，异名模块时追加，
        以免子表模型不参与 mapper 注册。

        参数:
        - gen_table (GenTableOutSchema): 生成表配置（含可选子表）。

        返回:
        - list[str]: 形如 ``["demo.model"]`` 的点分路径列表（相对插件根包）。
        """
        module_name = (gen_table.module_name or "").strip()
        paths: list[str] = []
        if module_name:
            paths.append(f"{module_name}.model")
        sub_table = gen_table.sub_table if (gen_table.sub and gen_table.sub_table) else None
        if sub_table is not None:
            sub_module_name = (sub_table.module_name or "").strip() or module_name
            if sub_module_name and sub_module_name != module_name:
                paths.append(f"{sub_module_name}.model")
        return paths

    @classmethod
    def format_python_tuple_literal(cls, values: list[str]) -> str:
        """把字符串列表渲染为符合 ``ruff format`` 的 Python 元组字面量。

        单元素用紧凑形态 ``("a",)``：单元素元组的逗号是语法必需，不属 magic trailing
        comma，格式化器会把它折叠成一行；多元素用展开形态，尾随逗号使格式化器保持展开。
        两者对称，保证生成物一次成型、不需要再跑格式化。

        参数:
        - values (list[str]): 元素列表。
