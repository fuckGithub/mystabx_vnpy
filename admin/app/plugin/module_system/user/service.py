...
from app.plugin.module_system.position.crud import PositionCRUD
from app.plugin.module_system.role.crud import RoleCRUD
...
class UserResolverImpl:
    """``auth.user_resolver`` 槽位实现。"""

    async def resolve(self, db: AsyncSession, username: str) -> UserModel | None:
        ...
        return await UserCRUD(auth).get_by_username_crud(
            username=username,
            preload=[
                "dept",
