...
│   ├── ⚙️ config/              # 项目配置文件
│   ├── 💖 core/                # 核心模块（数据库、中间件、安全、插件框架）
│   ├── 🔌 plugin/              # 业务插件（全部业务模块，按目录自动发现）
...
全部业务模块均为 **插件**，位于 `app/plugin/module_*/`，每个插件自带三件套：

```txt
module_*/
├── plugin.toml      # 插件清单（名称/版本/依赖/迁移）
├── plugin.py        # 模块级 PLUGIN = Plugin() 入口
...
OS 环境变量（最高）
  ↓ 覆盖
.env.{ENVIRONMENT}（如 .env.dev / .env.prod，按需创建，不提交 Git）
  ↓ 覆盖
.env（公共默认，提交 Git）
  ↓ 覆盖
代码默认值（setting.py 中各字段的 = 默认值，最低）
```

- 默认仅加载 `.env`，可直接启动
- 设置 `ENVIRONMENT=dev` 时额外加载 `.env.dev` 覆盖
- 设置 `ENVIRONMENT=prod` 时额外加载 `.env.prod` 覆盖
...
# 生成 requirements.txt（与 uv.lock 锁定版本一致，用于生产 pip 部署）
uv export --no-dev --no-header --no-emit-project -o requirements.txt
