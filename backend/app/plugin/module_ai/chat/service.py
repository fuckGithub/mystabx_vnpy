...
from app.plugin.module_system.dept.service import DeptService

# 模型配置服务：用于根据 model_id 构建模型实例
from ..model.service import AiModelService
...
class ChatService:
    ...
    async def chat_query(
        ...
        try:
            ...
            # 创建 AgnoFactory 实例并创建 Team，传入数据库连接与模型配置
            ...
            # 根据请求中的 model_id 构建模型实例（None 则用默认/兜底）
            model_instance = await AiModelService.build_model_by_id(
                auth=auth, id=query.model_id, enable_thinking=query.enable_thinking
            )

            # 构建 RAG 知识库与系统查询工具（延迟导入避免循环依赖）
            from ..rag import get_knowledge, get_vector_db
            from ..rag.knowledge_content import ensure_knowledge_loaded, rag_supported
            from ..rag.system_tools import get_system_tools

            system_tools = get_system_tools()
            # 非 PostgreSQL 后端无 pgvector：降级为仅注入系统查询工具
            knowledge = get_knowledge(get_vector_db()) if rag_supported() else None
            if knowledge is not None:
                # 确保知识文档已加载（幂等，进程内只加载一次）
                await ensure_knowledge_loaded(knowledge)
            ...
            # 执行聊天查询 - 使用流式输出
            # 思考内容（reasoning_content）与回答内容（content）分开推送
            async for chunk in agent.arun(input=query.message, stream=True):
                ...
                if reasoning:
                    yield f"__REASONING__{reasoning}__END__"
                if content:
                    yield content
    ...
    async def chat_non_stream(
        ...
        - auth (AuthSchema): 当前用户认证信息。
        - model_id (int | None): 模型配置ID；None 时使用默认模型。
        - enable_thinking (bool | None): 思考模式开关；None 时使用模型默认。
        ...
        try:
            ...
            # 创建 AgnoFactory 实例并创建 Team，传入数据库连接与模型实例
            agno_factory = AgnoFactory()
            dept_id = str(auth.user.dept_id) if auth and auth.user and hasattr(auth.user, 'dept_id') and auth.user.dept_id else "default"
            # 根据 model_id 构建模型实例（None 则使用默认/兜底模型）
            model_instance = await AiModelService.build_model_by_id(
                auth=auth, id=model_id, enable_thinking=enable_thinking
            )
