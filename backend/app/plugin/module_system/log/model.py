...
class OperationLogModel(ModelMixin, UserMixin):
    ...
    # ================================================== #
    # 账号 / 手机号 / 登录平台（冗余存储，日志自包含）       #
    # ================================================== #
    username: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="操作人账号（冗余）"
    )
    mobile: Mapped[str | None] = mapped_column(
        String(11), nullable=True, comment="操作人手机号（冗余）"
    )
    login_platform: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="登录平台(PC/FLUTTER/UNIAPP)"
    )

    # 国密 SM2 签名
    signature: Mapped[str | None] = mapped_column(
        String(256), nullable=True, comment="SM2签名值（国密完整性保护）"
    )
    signed_fields: Mapped[str | None] = mapped_column(
        String(512), nullable=True, comment="被签名字段标识（JSON格式）"
    )
