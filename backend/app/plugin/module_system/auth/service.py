...
from app.core.auth.schema import AuthSchema, JWTPayloadSchema
...
class LoginService:
    ...
    async def authenticate_user_service(
        ...
        if not PwdUtil.verify_password(
            ...
            raise CustomException(msg="账号或密码错误")
        log.info(f"[登录计时] 密码校验: {round((time.time() - _t3) * 1000, 1)}ms")
        _t4 = time.time()
    ...
    async def refresh_token_service(
        ...
        if not token_payload.is_refresh:
            raise CustomException(msg="非法凭证，请传入刷新令牌", code=10401, status_code=401)

        # 会话与用户来自 JWT 载荷本身（此处不查 Redis；Redis 只用于下面的吊销比对）
        ...
        if not session_id or not user_id:
            raise CustomException(
                msg="非法凭证,无法获取会话编号或用户ID", code=10401, status_code=401
            )

        # 刷新令牌必须与 Redis 中当前留存的那一个完全一致。
        # 登出（logout_service 删除该 key）与令牌轮换靠这一步才真正生效 ——
        # 否则旧刷新令牌仍能换出新访问令牌：登出形同虚设，且可被无限续期。
        # REDIS_ENABLE=False 时整条鉴权链路本就不可用（redis_connect 直接抛错），
        # 因此这里的严格比对不会误伤「无 Redis」部署。
        stored = await RedisCURD(redis).get(
            f"{RedisInitKeyConfig.REFRESH_TOKEN.key}:{session_id}"
        )
        if not stored or stored != refresh_token.refresh_token:
            raise CustomException(
                msg="刷新令牌已失效，请重新登录", code=10401, status_code=401
            )
    ...
    @classmethod
    async def logout_service(cls, redis: Redis, token: str) -> bool:
        """
        退出登录
...
class CaptchaService:
    ...
    @classmethod
    async def get_captcha_service(cls, redis: Redis) -> dict[str, bool | CaptchaKey | CaptchaBase64]:
        """
        获取验证码
        ...
        if not settings.CAPTCHA_ENABLE:
            return {
                "enable": False,
                "key": CaptchaKey("disabled"),
                "img_base": CaptchaBase64("disabled"),
            }
