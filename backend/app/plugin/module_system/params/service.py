...
class ParamsProviderImpl:
    """``config.params_provider`` 槽位实现。"""

    async def get_system_config(self, redis: Redis) -> dict:
        """读取中间件所需的系统配置。
        ...
        - dict: 配置字典。
        """
        return await ParamsService.get_system_config_for_middleware(redis)

    def set_param(self, key: str, value: str, name: str | None = None) -> None:
        """写入/更新一条系统参数（同步实现：供调度器等同步上下文调用）。
