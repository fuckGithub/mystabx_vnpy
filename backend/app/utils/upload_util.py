...
class UploadUtil:
    ...
    async def download_file(cls, file_path: str) -> str:
        """
        获取待下载文件的文件名（供 ``Content-Disposition`` 使用）。

        只做路径解析，**不读取文件内容**：内容由 ``FileResponse`` 自行流式读取。
        原实现误用 ``generate_file``（它产出的是文件内容分片迭代器，不是文件名），
        导致下载文件名变成 ``<generator object ...>``。

        保留 ``async`` 签名仅为兼容既有调用方，内部无 await。
