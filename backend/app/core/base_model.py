...
if TYPE_CHECKING:
    from app.core.auth.schema import UserLike
...
class UserMixin(MappedBase):
    ...
    @declared_attr
    def created_by(self) -> Mapped[UserLike | None]:
        """
        创建人关联关系（延迟加载，避免循环依赖）。

        ``relationship()`` 的目标由字符串名 ``"UserModel"`` 经 declarative 注册表解析
        （模型类名，见 ``MappedBase.registry``），故注解用内核的 ``UserLike`` 协议即可，
        内核无需 import 插件模型。

        返回:
        - Mapped[Optional[UserLike]]: 创建人 ORM 关系。
        """
        return relationship(
            "UserModel",
            lazy="selectin",
    ...
    @declared_attr
    def updated_by(self) -> Mapped[UserLike | None]:
        """
        更新人关联关系（延迟加载，避免循环依赖）。

        返回:
        - Mapped[Optional[UserLike]]: 更新人 ORM 关系。
        """
        return relationship(
            "UserModel",
            lazy="selectin",
    ...
    @declared_attr
    def deleted_by(self) -> Mapped[UserLike | None]:
        """
        删除人关联关系（延迟加载，避免循环依赖）。

        返回:
        - Mapped[Optional[UserLike]]: 删除人 ORM 关系。
        """
        return relationship(
            "UserModel",
            lazy="selectin",
