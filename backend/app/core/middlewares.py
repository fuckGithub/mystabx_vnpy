...
from app.core.plugin.slots import SLOT_CONFIG_PARAMS_PROVIDER, get
...
class RequestLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        try:
            ...
            try:
                # 系统配置走内核槽位：槽位缺失＝提供方插件未安装（最小部署可缺）
                # → 直接把上方预初始化的默认值（演示模式关闭 / 三个名单为空）原样用于
                # 后续拦截判断，静默降级且不在热路径记日志（启动期由 lifespan 统一告警）。
                params_provider = get(SLOT_CONFIG_PARAMS_PROVIDER)
                if params_provider is not None:
                    ...
                    system_config = await params_provider.get_system_config(redis)
                    # 提取配置值
