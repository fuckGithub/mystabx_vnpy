...
from app.core.plugin.slots import SLOT_AUTH_DATA_SCOPE_MODELS, get
...
class Permission:
    ...
    def _models(self) -> dict[str, Any] | None:
        """取数据权限所需的模型（通过槽位提供）。

        返回:
        - dict[str, Any] | None: ``{"dept_model": ..., "user_model": ...}``；未提供时为 None。
        """
        # SLOT: auth.data_scope_models
        return get(SLOT_AUTH_DATA_SCOPE_MODELS)
    ...
    async def __filter_by_dept_based(self) -> ColumnElement | None:
        ...
        dept_model = models["dept_model"]
        user_model = models["user_model"]

        # 根据模型类型过滤
        if self.model.__name__ == dept_model.__name__:
            return self.__filter_dept_model(accessible_dept_ids)
        elif self.model.__name__ == user_model.__name__:
    ...
    async def __filter_by_data_scope(self) -> ColumnElement | None:
        ...
        if accessible_dept_ids:
            # 特殊处理：如果模型本身就是用户模型，直接过滤用户的dept_id
            models = self._models()
            user_model_cls = models["user_model"] if models else None
            user_model_name = user_model_cls.__name__ if user_model_cls else ""
            if self.model.__name__ == user_model_name and hasattr(self.model, "dept_id"):
            ...
            if creator_rel is not None and models and hasattr(models["user_model"], "dept_id"):
                user_model = models["user_model"]
                return creator_rel.has(user_model.dept_id.in_(list(accessible_dept_ids)))
    ...
    async def __get_accessible_dept_ids(
        ...
        if self.DATA_SCOPE_DEPT_AND_CHILD in data_scopes and user_dept_id is not None:
            try:
                models = self._models()
                dept_model = models["dept_model"] if models else None
                if dept_model is None:
                    accessible_dept_ids.add(user_dept_id)
                else:
                    dept_sql = select(dept_model)
