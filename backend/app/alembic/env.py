...
from app.core.plugin.runtime import migration_locations
...
# 聚合核心与全部插件的迁移目录：插件自带迁移时也能被 alembic 发现，
# 从而保证 `alembic heads` 只暴露同一分支的单一 head。
alembic_config.set_main_option("version_locations", os.pathsep.join(migration_locations()))
